...
for theta=[0,1/2-1/(12*6/5),1/2,1]
    ...
    for n=3:7
        ...
        sol=[exp(-pi^2*0.1).*sin(pi.*x);exp(-pi^2).*sin(pi.*x);exp(-pi^2*10).*sin(pi.*x)];%真解
    ...
    saveas(gcf,strcat('theta=',num2str(theta),',N_e.jpg'));
    clf;
    ...
    saveas(gcf,strcat('theta=',num2str(theta),',iter_e.jpg'));
    clf;
end